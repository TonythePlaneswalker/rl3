...
class Reinforce(object):
    ...
    def train(self, env, gamma=0.99):
        ...
        # writer = tf.summary.FileWriter('./log/%s'.format(args.log_dir), self.sess.graph)
        ...
        for i in range(self.num_episodes):
            ...
            # train the model
            _, cur_policy, cur_loss, cur_summaries = sess.run([self.train_op, self.policy, self.loss, merged],
                                                              feed_dict={self.state: states,
                                                                         self.action: actions,
                                                                         self.G: G})
            # print("Loss: {}".format(cur_loss))
            # train_writer.add_summary(cur_summaries)
            if (i + 1) % 100 == 0:
